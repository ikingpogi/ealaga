...
// console.log(transporter);
    ...
    try {
        const { first_name, middle_name, last_name, user_name, 
            email, password, confirmPassword, role, status} = req.body;
        ...
        const user = await User.create({
            ...
            user_name, email, password, role, status,
            profile_picture: {
...
exports.profileUpdate = catchAsyncErrors(async(req,res,next) => {
    ...
    const { first_name, middle_name, last_name, age, 
      birth_date, house_purok_no, street, barangay, gender, valid_id, confirm,health_id} = req.body;

      const { profile_picture} = req.files;


    const userss = await User.findByIdAndUpdate(id,{
        $unset: { health_id:""}
    }, 
    {
        new: true,
    ...
    const newUserData = {
      ...
      address:{house_purok_no: house_purok_no,
        street: street, 
        barangay: barangay },
      gender: gender,
      health_id: health_id,
...
exports.profileUpdateSubmit = catchAsyncErrors(async(req,res,next) => {
  ...
  const { first_name, middle_name, last_name, age, 
    birth_date, house_purok_no, street, barangay, gender } = req.body;
...
exports.profileupdateCredential = catchAsyncErrors(async (req, res, next) => {
  const { id } = req.params;
  const { user_name,phone,email,oldPassword, newPassword, confirmPassword } = req.body;
  ...
  // console.log(id, user_name,phone,email,oldPassword, newPassword, confirmPassword)
    ...
    const usernames = await User.find({'user_name': {$ne : user.user_name}}).exec();

    var userAllEmail = emails.map(function(emailsss){return emailsss.email});
    var userAllUsername = usernames.map(function(usernamessss){return usernamessss.user_name});
...
exports.profileUpdateHealth = catchAsyncErrors(async (req, res, next) => {
  ...
  if(!req.files) {
    ...
    const userss = await User.findByIdAndUpdate(id,{
      ...
      const user = await User.findByIdAndUpdate(id, {health_id: req.body.health_id}, {
          new: true,
          runValidators: true,
          useFindAndModify: false
      })
  ...
  const userss = await User.findByIdAndUpdate(id,{
    ...
    const userMedical_certificate = await User.findByIdAndUpdate(id,{medical_certificate: {
      public_id: fileId,
      url: `https://drive.google.com/uc?export=view&id=${fileId}`
    }}, 
    {
...
exports.acceptAttendees = catchAsyncErrors(async (req, res, next) => {
 ...
 const user = await Schedule.findByIdAndUpdate(id, {status: "attended"}, {
  new: true,
  runValidators: true,
  useFindAndModify: false
})
...
exports.QRacceptAttendees = catchAsyncErrors(async (req, res, next) => {
 ...
 const user = await Schedule.findByIdAndUpdate(mongoose.Types.ObjectId(str), {status: "attended"}, {
  new: true,
  runValidators: true,
  useFindAndModify: false
})
